...
function generateNextWorkout(user: UserProfile): WorkoutDay | null {
    ...
    // Sort clusters by LOW volume count first (prioritize <20 sets/7days), then by oldest timestamp
    readyClusters.sort((a, b) => {
        const clusterA = TRAINING_CLUSTERS[a as keyof typeof TRAINING_CLUSTERS];
        const clusterB = TRAINING_CLUSTERS[b as keyof typeof TRAINING_CLUSTERS];

        // Calculate total 7-day volume for each cluster
        let volumeA = 0, volumeB = 0;
        for (const muscle of clusterA.muscles) {
            volumeA += (status.rolling7DayVolume as any)?.[muscle] || 0;
        }
        for (const muscle of clusterB.muscles) {
            volumeB += (status.rolling7DayVolume as any)?.[muscle] || 0;
        }

        // Prioritize clusters with lower volume (need more work)
        if (volumeA !== volumeB) {
            return volumeA - volumeB; // Lower volume first
        }

        // If volumes equal, sort by oldest timestamp
        let avgA = 0, avgB = 0;
        for (const muscle of clusterA.muscles) {
    ...
    selectedClusters.forEach(clusterKey => {
        ...
        for (const muscle of cluster.muscles) {
            ...
            if (muscle === 'chest') {
                ...
                const preExNotes = isPastFailureWeek
                    ? `${rirMessage}\n\n${getIntensificationTechnique('preExhaust')}`
                    : rirMessage;
                const mainNotes = isPastFailureWeek
                    ? `${rirMessage}\n\n${getIntensificationTechnique('main')}`
                    : rirMessage;
                const finishNotes = isPastFailureWeek
