    ...
    if (!user.houseOfIronStatus?.equipment?.length) return (
        <div className="instrument-page max-w-2xl space-y-6">
            ...
            <div className="grid sm:grid-cols-3 gap-4">
                <label className="text-xs uppercase tracking-widest text-muted-foreground">{pl ? 'Typ' : 'Type'}<select value={type} onChange={event => setType(event.target.value as typeof type)} className="block w-full min-h-11 bg-transparent border-b border-input text-foreground text-base"><option value="dumbbell">{pl ? 'Hantel' : 'Dumbbell'}</option><option value="kettlebell">Kettlebell</option></select></label>
                <label className="text-xs uppercase tracking-widest text-muted-foreground">{pl ? 'Liczba' : 'Count'}<select value={count} onChange={event => setCount(Number(event.target.value) as 1 | 2)} className="block w-full min-h-11 bg-transparent border-b border-input text-foreground text-base"><option value={1}>{pl ? 'Jedna sztuka' : 'One'}</option><option value={2}>{pl ? 'Para' : 'Pair'}</option></select></label>
            ...
            {new Set(equipment.map(item => item.type)).size > 1 && <label className="block text-xs uppercase tracking-widest text-muted-foreground">{pl ? 'Preferowany typ' : 'Preferred type'}<select value={preferred} onChange={event => setPreferred(event.target.value as typeof preferred)} className="block w-full min-h-11 bg-transparent border-b border-input text-foreground text-base"><option value="dumbbell">{pl ? 'Hantel' : 'Dumbbell'}</option><option value="kettlebell">Kettlebell</option></select></label>}
