        ...
        <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
            <DialogContent className="max-w-lg bg-card border-border text-foreground">
                ...
                <div className="space-y-6 py-4">
                    {/* Tip */}
                    <div className="bg-secondary/50 border border-border rounded p-3 text-sm text-foreground">
                        <strong className="text-foreground">{t('trinary.weakPointModal.tipTitle')}</strong><br />
                        {t('trinary.weakPointModal.tipText')}
                    </div>

                    {/* Bench Press */}
                    <div className="space-y-3">
                        <h3 className="font-bold text-foreground">{t('trinary.weakPointModal.benchTitle')}</h3>
                        <RadioGroup value={benchWeak} onValueChange={(v) => setBenchWeak(v as any)}>
                            <div className="space-y-2">
                                <div className="flex items-center space-x-2">
                                    ...
                                    <Label htmlFor="bench-off-chest" className="text-foreground cursor-pointer">
                                        {t('trinary.weakPointModal.benchOffChest')}
                                    </Label>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="mid-range" id="bench-mid-range" />
                                    <Label htmlFor="bench-mid-range" className="text-foreground cursor-pointer">
                                        {t('trinary.weakPointModal.benchMidRange')}
                                    </Label>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="lockout" id="bench-lockout" />
                                    <Label htmlFor="bench-lockout" className="text-foreground cursor-pointer">
                    ...
                    {/* Squat */}
                    <div className="space-y-3">
                        <h3 className="font-bold text-foreground">{t('trinary.weakPointModal.squatTitle')}</h3>
                        <RadioGroup value={squatWeak} onValueChange={(v) => setSquatWeak(v as any)}>
                            <div className="space-y-2">
                                <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="bottom" id="squat-bottom" />
                                    <Label htmlFor="squat-bottom" className="text-foreground cursor-pointer">
                                        {t('trinary.weakPointModal.squatBottom')}
                                    </Label>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="mid-range" id="squat-mid-range" />
                                    <Label htmlFor="squat-mid-range" className="text-foreground cursor-pointer">
                                        {t('trinary.weakPointModal.squatMidRange')}
                                    </Label>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="lockout" id="squat-lockout" />
                                    <Label htmlFor="squat-lockout" className="text-foreground cursor-pointer">
