...
for (const [name, days] of [['4-day', VENUS_FOUR_DAY], ['3-day', VENUS_THREE_DAY]] as const) {
    ...
    for (const day of days) {
        ...
        ok(sets >= 15 && sets <= 19, `${name} ${day.name} stays at 15–19 sets`);
    }
}
...
for (const day of week12) {
    ...
    ok(ratio >= 0.55 && ratio <= 0.7, `${day.dayName} rebirth volume is 55–70%`);
}
...
ok(risingDay.exercises.reduce((sum, ex) => sum + ex.sets, 0) <= 19, 'priority additions cannot exceed session cap');
